...
const AllDoc = () => {
    ...
    const sendEmail = (e) => {
        e.preventDefault();
        console.log('Form Data:', formData); // Check the formData object before sending
                    ...
                    {currentDoctor.map((dr, id) => (
                        <div key={id} className="bg-blue-900 rounded-lg shadow-md p-4 mb-4 text-white ">
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <button  className="button-2 font-semibold border border-indigo-500 bg-white hover:bg-blue-900 hover:text-white text-black px-4 py-2 rounded-lg transition duration-300" onClick={()=>handleOpen(dr)}>Online Appointment</button>
                            </div>
                        </div>
                    ))}
                </div>
                    ...
                    <div className="bg-gray-200 rounded-lg shadow-md p-4 mb-10">
                        ...
                        <form ref={form} onSubmit={sendEmail}>
                            <div className="mb-4">
                                ...
                                <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} className="mt-1 p-2 w-full border rounded-md" placeholder="Enter your name" />
                            </div>
                            <div className="mb-4">
                                <label htmlFor="phone" className="block text-sm font-medium text-gray-700">Phone</label>
                                <input type="text" id="phone" name="phone" value={formData.phone} onChange={handleChange} className="mt-1 p-2 w-full border rounded-md" placeholder="Enter your phone number" />
                            </div>
                            <div className="mb-4">
                                <label htmlFor="city" className="block text-sm font-medium text-gray-700">City</label>
                                <input type="text" id="city" name="city" value={formData.city} onChange={handleChange} className="mt-1 p-2 w-full border rounded-md" placeholder="Enter your city" />
                            </div>
